...
import no.nav.helse.sparkel.personopplysninger.oppslag.arbeidsforhold.ArbeidsforholdClient
...
class ArbeidsforholdIntegrationTest {
    ...
    fun `skal svare med en liste over arbeidsgivere`() {
        ...
        arbeidsforholdStub(
                ...
                request = finnArbeidsforholdPrArbeidstakerStub(aktørId, fom.toXmlGregorianCalendar().toXMLFormat(), tom.toXmlGregorianCalendar().toXMLFormat()),
    ...
    fun `skal svare med feil når tjenesten svarer med feil`() {
        ...
        arbeidsforholdStub(
                ...
                request = finnArbeidsforholdPrArbeidstakerStub(aktørId, fom.toXmlGregorianCalendar().toXMLFormat(), tom.toXmlGregorianCalendar().toXMLFormat()),
                response = WireMock.serverError().withHeader("Content-Type", "text/xml;charset=UTF-8").withBody(ugyldigAktørResponse(aktørId))
        ) { arbeidsforholdClient ->
            val actual = arbeidsforholdClient.finnArbeidsforhold(aktørId, fom, tom)
